import type { GameState, Action, AvailableActions } from '../types/poker';
...
export class Room {
  ...
  private addPlayer(playerId: string, name: string) {
    if (this.players.has(playerId)) {
      ...
      this.players.set(playerId, {
        id: playerId,
        name,
        ws: null as any, // Will be set by handleConnection
        seatIndex: null,
        disconnectedAt: null,
      });
    }
  ...
  private handleAction(playerId: string, actionType: string, amount?: number) {
    ...
    try {
      ...
      this.sendTo(playerId, { type: 'error', message: e.message });
    }
  }
  ...
  private handleChat(playerId: string, text: string) {
    const player = this.players.get(playerId);
    if (!player) return;
    const sanitized = text.slice(0, 200);
    this.broadcast({ type: 'chat', playerId, name: player.name, text: sanitized });
  }
